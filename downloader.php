...
$domain_path = ""; /* example : https://download.com/files/ */
...
if($url == "/start"){
	...
	if(filter_var($url, FILTER_VALIDATE_URL)){
		...
		if(remote_file_size($url) < 50){
			...
			bot('sendMessage', ['chat_id'=> $user_id,'text'=> $domain_path.$fileName]);
		}
	}
}
...
function send_file($user_id, $fileName){
	...
	$post = array("document" => new CURLFile(realpath('files/'.$fileName)));
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, $url);
